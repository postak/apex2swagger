...
DECLARE
  ...
  c_workspace CONSTANT varchar(100)    := 'postak';
  c_basepath CONSTANT varchar(100)     := '/ords/postak';
  c_description CONSTANT varchar(100) := 'this is the description';
  c_title CONSTANT varchar(100)       := 'this is the title';
  c_hostname CONSTANT varchar(100)     := 'localhost';
  c_port CONSTANT number(4)            := 8080;
  ...
  CURSOR cur_services is
      SELECT APEX_REST_RESOURCE_HANDLERS.workspace ,
            APEX_REST_RESOURCE_TEMPLATES.module_name, method,
            APEX_REST_RESOURCE_MODULES.uri_prefix,
           APEX_REST_RESOURCE_TEMPLATES.uri_template, source_type, 
           source , format
            FROM  APEX_REST_RESOURCE_HANDLERS, APEX_REST_RESOURCE_TEMPLATES,APEX_REST_RESOURCE_MODULES 
            WHERE APEX_REST_RESOURCE_HANDLERS.template_id = 
              APEX_REST_RESOURCE_TEMPLATES.template_id AND 
